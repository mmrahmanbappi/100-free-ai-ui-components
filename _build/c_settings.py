...
function status(msg,cls){var n=ta.value.length;st.innerHTML=n.toLocaleString()+' / 1,500'+(msg?' <span class="'+cls+'">'+msg+'</span>':'');}
function check(){var dirty=ta.value!==saved;sv.disabled=!dirty;status(dirty?'Unsaved changes':'','dirty');}
...
sv.addEventListener('click',function(){saved=ta.value;sv.disabled=true;status('Saved','saved');});
